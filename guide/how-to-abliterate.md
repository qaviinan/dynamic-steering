...
In this example, we'll use `mlabonne/Daredevil-8B`, a mega-merge created with DARE TIES that has the highest MMLU score on the Open LLM Leaderboard in the 8B category.
...
Everything is set up, we can now implement the first step of abliteration: data collection. We want to process these tokenized datasets and store the residual stream activations in `harmful` and `harmless`. This is managed by the `transformer_lens` library.
...
def _generate_with_hooks(
    ...
    for i in range(max_tokens_generated):
        with model.hooks(fwd_hooks=fwd_hooks):
            ...
            all_tokens[:, -max_generated + i] = next_tokens
...
I hope you liked this article. If you want to see more follow me on Hugging Face and Twitter [@maximelabonne](https://twitter.com/maximelabonne).